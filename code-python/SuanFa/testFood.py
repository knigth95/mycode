...
def main():
    ...
    # 目标函数：最小化（价格归一化）食品的总和.
    objective = solver.Objective()#.Minimize()
    for food in foods:#, 枚举中的项（数据）:
        objective.SetCoefficient(food, 1)# food.Maximize())
    objective.SetMinimization()# 设置目标函数以最小化.
    status = solver.Solve()# Solve the problem.
    ...
    # 显示购买每种食物的金额（以美元为单位）.
    nutrients_result = [0] * len(nutrients)# [0] * len(nutrients)
    print('\nAnnual Foods:')
